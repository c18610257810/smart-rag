...
export class RAGEngine {
  ...
  private serverUrl: string;  // LightRAG server URL
  ...
  constructor(
    ...
    this.serverUrl = serverUrl || 'http://127.0.0.1:9621'
  ...
  async insertDocument(content: string, description?: string): Promise<boolean> {
    ...
    try {
      const response = await requestUrl({
          url: `${this.serverUrl}/documents/texts`,
          method: "POST",
  ...
  async uploadDocument(file: TFile): Promise<boolean> {
    try {
      ...
      const response = await requestUrl({
        url: `${this.serverUrl}/documents/upload`,
        method: "POST",
        headers: {
