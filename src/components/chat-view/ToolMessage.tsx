...
const ToolMessage = memo(function ToolMessage({
  ...
  return (
    <div className="smart-rag-toolcall-container">
      {message.toolCalls.map((toolCall, index) => (
        <div
          key={toolCall.request.id}
          className={clsx(index > 0 && 'smart-rag-toolcall-border-top')}
        >
          <ToolCallItem
...
function ToolCallItem({
  ...
  return (
    <div className="smart-rag-toolcall">
      <div
        onClick={() => setIsOpen(!isOpen)}
        className="smart-rag-toolcall-header"
      >
        <div className="smart-rag-toolcall-header-icon">
          {isOpen ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
        </div>
        <div className="smart-rag-toolcall-header-content">
          <span>{STATUS_LABELS[response.status] || 'Unknown'}</span>
          <span>&nbsp;&nbsp;</span>
          <span className="smart-rag-toolcall-header-tool-name">
            {serverName ? `${serverName}:${toolName}` : toolName}
          </span>
        </div>
        <div className="smart-rag-toolcall-header-icon smart-rag-toolcall-header-icon--status">
          <StatusIcon status={response.status} />
        </div>
      </div>
      {isOpen && (
        <div className="smart-rag-toolcall-content">
          <div className="smart-rag-toolcall-content-section">
            <div>Parameters:</div>
      ...
      {(response.status === ToolCallResponseStatus.PendingApproval ||
        ...
        <div className="smart-rag-toolcall-footer">
          {response.status === ToolCallResponseStatus.PendingApproval && (
            <div className="smart-rag-toolcall-footer-actions">
              <SplitButton
                primaryText="Allow"
                onPrimaryClick={() => {
                  void handleToolCall()
...
function StatusIcon({ status }: { status: ToolCallResponseStatus }) {
  switch (status) {
    ...
    case ToolCallResponseStatus.Error:
      return <X size={16} className="smart-rag-text-error" />
    case ToolCallResponseStatus.Running:
      return <Loader2 size={16} className="spinner" />
    case ToolCallResponseStatus.Success:
      return <Check size={16} className="smart-rag-text-success" />
    default:
      return null
  }
}
