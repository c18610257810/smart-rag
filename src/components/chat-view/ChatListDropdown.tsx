...
function TitleInput({
  ...
  return (
    <input
      ...
      type="text"
      value={value}
      className="smart-rag-chat-list-dropdown-item-title-input"
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <div className="smart-rag-chat-list-dropdown-item-title">{title}</div>
      )}
      <div className="smart-rag-chat-list-dropdown-item-actions">
        <button
          onClick={(e) => {
            e.stopPropagation()
            onStartEdit()
          }}
          className="clickable-icon smart-rag-chat-list-dropdown-item-icon"
        >
          <Pencil size={16} />
        </button>
        <button
...
export function ChatListDropdown({
  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      ...
      <Popover.Portal>
        <Popover.Content
          className="smart-rag-popover smart-rag-chat-list-dropdown-content"
          onKeyDown={handleKeyDown}
        >
          <ul>
            {chatList.length === 0 ? (
              <li className="smart-rag-chat-list-dropdown-empty">
                No conversations
              </li>
            ) : (
