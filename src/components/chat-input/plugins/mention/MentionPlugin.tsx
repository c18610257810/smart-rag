...
function MentionsTypeaheadMenuItem({
  ...
  return (
    <li
      ...
      onClick={onClick}
    >
      {Icon && <Icon size={14} className="smart-rag-popover-item-icon" />}
      <span className="text">{option.name}</span>
      {option.mentionable.type === 'folder' && (
        <span className="text smart-rag-mention-popover-folder-path">
    ...
    <LexicalTypeaheadMenuPlugin<MentionTypeaheadOption>
      ...
      menuRenderFn={(
        ...
        anchorElementRef.current && results.length
          ? createPortal(
              <div
                className="smart-rag-popover smart-rag-mention-popover"
                style={{
                  position: 'fixed',
                }}
              >
