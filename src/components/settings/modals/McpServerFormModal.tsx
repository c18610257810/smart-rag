...
- "env": (Optional) Key-value pairs of environment variables`}
        className="smart-rag-settings-textarea-header smart-rag-settings-description-preserve-whitespace"
        required
      />
      <TextareaAutosize
        value={parameters}
        ...
        className="smart-rag-mcp-server-modal-textarea"
        maxRows={20}
        minRows={PARAMETERS_PLACEHOLDER.split('\n').length}
      />
      {validationError !== null ? (
        <div className="smart-rag-mcp-server-modal-validation smart-rag-mcp-server-modal-validation--error">
          {validationError}
        </div>
      ) : (
        <div className="smart-rag-mcp-server-modal-validation smart-rag-mcp-server-modal-validation--success">
          Valid parameters
        </div>
      )}
