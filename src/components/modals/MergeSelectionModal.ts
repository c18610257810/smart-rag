...
export class MergeSelectionModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl('p', { 
        ...
        cls: 'smart-rag-merge-instruction'
    });

   // 2. Caja de Advertencia
    const warningBox = contentEl.createDiv({ cls: 'smart-rag-merge-warning' });
    
    // Párrafo 1: Advertencia de deshacer (Todo en negrita para énfasis)
    const p1 = warningBox.createEl("p", { cls: 'smart-rag-warning-line' });
    ...
    const p2 = warningBox.createEl("p", { cls: 'smart-rag-warning-line' });
    ...
    const listContainer = contentEl.createDiv({ cls: 'smart-rag-merge-list' });

    // Referencia inicializada como null/undefined explícitamente para claridad
    let submitButton: ButtonComponent | undefined;

    this.selectedNodes.forEach((node) => {
        const row = listContainer.createDiv({ cls: 'smart-rag-merge-item' });
        ...
        rb.addClass('smart-rag-merge-radio');
        ...
        label.htmlFor = rbId;
        label.addClass('smart-rag-merge-label');
    });

    // 4. Botonera
    const buttonDiv = contentEl.createDiv({ cls: 'smart-rag-merge-actions' });
