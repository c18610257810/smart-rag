...
export class CreateRelationModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.createEl('label', { text: 'Select source node (the hub):', cls: 'smart-rag-modal-label' });
    ...
    // --- LISTA DE OBJETIVOS ---
    contentEl.createEl('label', { text: 'Target nodes:', cls: 'smart-rag-modal-label' });
    const targetList = contentEl.createDiv({ cls: 'smart-rag-modal-input-container' });
    // Fix: Use setCssProps
    targetList.setCssProps({ "font-size": "0.85em" });
    targetList.setText("All other selected nodes will be linked to the source.");

    // --- DESCRIPCIÓN ---
    contentEl.createEl('label', { text: 'Relationship description:', cls: 'smart-rag-modal-label' });
    ...
    // --- KEYWORDS ---
    contentEl.createEl('label', { text: 'Keywords (comma separated):', cls: 'smart-rag-modal-label' });
    ...
    // --- BOTONERA FINAL ---
    const buttonDiv = contentEl.createDiv({ cls: 'smart-rag-edit-actions' });
    // Fix: Use setCssProps
